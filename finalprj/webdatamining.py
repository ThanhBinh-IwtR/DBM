...
if selected_main_menu == "Data Information":
    ...
    if st.session_state["Predict"]:
        st.header("Predict Total Emissions")

        year = st.number_input("Year", min_value=1900, max_value=2100, value=2021)
        parent_entity = st.selectbox("Parent Entity", raw["parent_entity"].unique())
        parent_type = st.selectbox("Parent Type", raw["parent_type"].unique())
        commodity = st.selectbox("Commodity", raw["commodity"].unique())
        ...
        if st.button("Predict"):
            ...
            new_data['year'] = new_data['year'] - raw['year'].min()
            max_year = raw['year'].max()
            new_data['year_diff'] = max_year - new_data['year']

            # Preprocess the new data using the label encoders
            for column in ['parent_entity', 'parent_type', 'commodity']:
                le = LabelEncoder()
                le.fit(raw[column])
                new_data[column] = le.transform(new_data[column])

            # Load the models
            bst_pv_loaded, trained_columns_pv = joblib.load('model_production_value.pkl')
            bst_te_loaded, trained_columns_te = joblib.load('model_total_emissions.pkl')

            # Ensure the new data has the same columns as the training data for production value prediction
            missing_columns_pv = set(trained_columns_pv) - set(new_data.columns)
            for column in missing_columns_pv:
                new_data[column] = np.nan

            # Predict production value
            new_data['production_value'] = bst_pv_loaded.predict(new_data[trained_columns_pv])

            # Ensure the new data has the same columns as the training data for total emissions prediction
            missing_columns_te = set(trained_columns_te) - set(new_data.columns)
            for column in missing_columns_te:
                new_data[column] = np.nan

            # Predict total emissions
            y_pred_new = bst_te_loaded.predict(new_data[trained_columns_te])

            st.write(f'Predicted Production Value: {new_data["production_value"][0]}')
            st.write(f'Predicted Total Emissions CO2: {y_pred_new[0]}')
